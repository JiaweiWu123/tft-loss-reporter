...
const { DISCORD_TOKEN, RIOT_API_KEY } = process.env;
...
const MATCH_IDS_URL = (puuid: string) =>
	`${AMERICAS_URL}/match/v1/matches/by-puuid/${puuid}/ids?api_key=${RIOT_API_KEY}`;
...
client.on('interactionCreate', async (interaction) => {
	...
	if (interaction.commandName === 'list') {
		...
		const names = leagueUserNames.map((name) => name.leagueName).join('\n');
...
cron.schedule('*/2 * * * *', async () => {
	...
	users.forEach(async (user: UserAttributes) => {
		...
		if (tftRank) {
			embed.addField('Tier', `${tftRank.tier} ${tftRank.rank}`).addField('LP', `${tftRank.leaguePoints}`);
		}

		embed.setFooter({
			text: `Check out his new ranking: https://lolchess.gg/search?region=NA&name=${user.leagueName}`,
		});
